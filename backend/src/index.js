...
async function connectDB() {
  try {
    ...
    console.warn('⚠️  Could not reach MongoDB Atlas — starting local in-memory database instead.');
    const { MongoMemoryServer } = require('mongodb-memory-server');
    const mongod = await MongoMemoryServer.create();
    await mongoose.connect(mongod.getUri());
    console.log('✅ In-memory MongoDB started (seed data loaded)');
    // Auto-seed so the app has data to work with
    const crypto = require('crypto');
    const hashPin = p => crypto.createHash('sha256').update(String(p)).digest('hex').slice(0, 16);
    ...
    await WorkerAlert.insertMany([
      { id:'wa1', workerId:'u1', type:'rain', title:'Rain today', message:"Orders may slow. You're fully covered.", badge:'AUTO ON' },
    ]);
    await AdminSettings.create({ key:'singleton', triggers:{ rain:{enabled:true,autoPay:true,payoutAmount:300,label:'Rain'}, lowOrders:{enabled:true,autoPay:false,payoutAmount:200,label:'Low Orders'}, curfew:{enabled:true,autoPay:true,payoutAmount:250,label:'Curfew'}, outage:{enabled:false,autoPay:false,payoutAmount:150,label:'Platform Outage'} }, gigtrust:{highMin:700,mediumMin:500}, premium:{minPremium:20,maxPremium:50,coverageDivisor:1800}, platform:{active:true,maintenanceMode:false,fraudAutoSuspend:true} });
  }
}
connectDB();
